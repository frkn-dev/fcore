...
use super::super::super::{
    ...
    sync::{tasks::{SyncOp, DELETE_REASON_MANUAL}, MemSync},
};
    ...
    Connection: From<C>,
{
    ...
    if let Err(e) = SyncOp::delete_connection(memory, child_id, &conn, Some(DELETE_REASON_MANUAL)).await {
        error!(
    ...
    S: SubscriptionOperations + Send + Sync + Clone + 'static + PartialEq,
{
    ...
    match build_gateway_config_response(&memory, &row.subscription_id, &params, false, &[]).await {
        Ok(mut resp) => {
