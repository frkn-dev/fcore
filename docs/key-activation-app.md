...
| `lite` | N ГиБ трафика, без срока | Подписка без `expires_at`, живёт пока есть трафик. `email` опционален — если передан, к нему привязывается аккаунт |
...
- `email`: опционален. Для lite-ключей, если передан — к нему привязывается
  аккаунт (уведомления о кончающемся трафике, восстановление доступа). Без
  email активация пройдёт, но системные письма приходить не будут. Для
  standard игнорируется.
